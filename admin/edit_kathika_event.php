...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $sql = "SELECT * FROM kathika_event WHERE id=?";
...
if(isset($_POST['update'])){
    ...
    if(isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK){
        ...
        if(move_uploaded_file($imageTmpName, $newFilePath)){
            ...
            $sql = "UPDATE kathika_event SET heading=?, image_path=?, description=? WHERE id=?";
            $stmt = $dbh->prepare($sql);
        ...
        $sql = "UPDATE kathika_event SET heading=?, description=? WHERE id=?";
        $stmt = $dbh->prepare($sql);
...
<head>
    ...
    <title>Edit Kathika  Event</title>
...
<body>
    <div class="container">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <a href="manage_kathika_event.php" class="btn btn-secondary">Cancel</a>
        </form>
    </div>
</body>
</html>
