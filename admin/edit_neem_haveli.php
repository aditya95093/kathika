...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $sql = "SELECT * FROM neem_haveli WHERE id=?";
...
if(isset($_POST['update'])){
    ...
    if(isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK){
        ...
        if(move_uploaded_file($imageTmpName, $newFilePath)){
            ...
            $sql = "UPDATE neem_haveli SET heading=?, image=?, description=? WHERE id=?";
            $stmt = $dbh->prepare($sql);
        ...
        $sql = "UPDATE neem_haveli SET heading=?, description=? WHERE id=?";
        $stmt = $dbh->prepare($sql);
